...
from utils.ontologies import NS_CODE, NS_DATA, NS_ONTO
...
def preprocess_meds_kg(
    ...
    with open(dcfg.triples_path, "w", buffering=1024 * 1024) as out:
        ...
        for h, r, t in iter:
            ...
            elif r == str(NS_ONTO["time"]):
                if ecfg.time_option == "NT":
                    continue
                try:
...
def _store_arrays(
    ...
    if ecfg.include_text:
        ...
        embeddings = np.zeros((len(text_values), 32), dtype=np.float32)

        projection = Linear(384, 32)
